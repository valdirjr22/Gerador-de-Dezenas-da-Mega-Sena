...
05 09 15 24 25 60`
    ...
    // State to store all parsed draws with their dates
    const [allParsedDraws, setAllParsedDraws] = useState([]);
    // State to store the filtered draws based on selected month/year
    const [filteredDraws, setFilteredDraws] = useState([]);
    // State to store the frequency counts of each number from filtered draws
    const [frequencyCounts, setFrequencyCounts] = useState([]);
    // State to store the generated Mega-Sena games
    const [generatedGames, setGeneratedGames] = useState([]);
    // State for the number of games to generate
    const [numGamesToGenerate, setNumGamesToGenerate] = useState(1);
    // State for the generation strategy ('frequencia' or 'ponderada')
    const [generationStrategy, setGenerationStrategy] = useState('frequencia');
    ...
    /**
     * Parses the raw input string of past draws into a structured array of objects,
     * including the date for each draw.
     * @param {string} input The raw text input from the user.
     * @returns {Array<{date: Date, numbers: Array<number>}>} An array of objects, each representing a draw.
     */
    const parseAllDraws = (input) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            // Regex to find the date in "DD/MM/YYYY" format
            ...
            if (line.startsWith('Concurso:') && dateMatch) {
                ...
                // Move to the next line to get the numbers
                if (i + 1 < lines.length) {
                    const numbersLine = lines[++i]; // Increment i to consume the next line
                    ...
                    errors.push(`Erro: Linha de concurso encontrada sem números correspondentes no final do input.`);
                }
            }
        }
    ...
    /**
     * Calculates the frequency of each number from the provided draws.
     * @param {Array<{date: Date, numbers: Array<number>}>} draws The array of draws to analyze.
     * @returns {Array<[number, number]>} A sorted array of [number, frequency] pairs.
     */
    const calculateFrequency = (draws) => {
        const counts = new Map();
        // Initialize all numbers from 1 to 60 with a frequency of 0
        for (let i = 1; i <= 60; i++) {
            counts.set(i, 0);
        }
        ...
        // Convert map to array and sort by frequency (descending) then by number (ascending)
        return Array.from(counts.entries()).sort((a, b) => {
            if (b[1] !== a[1]) {
                return b[1] - a[1]; // Sort by frequency (descending)
            }
            return a[0] - b[0]; // Then by number (ascending)
        });
    };

    // Effect to parse all draws when input changes
    useEffect(() => {
        const parsed = parseAllDraws(pastDrawsInput);
        setAllParsedDraws(parsed);
    }, [pastDrawsInput]);

    // Effect to filter draws and calculate frequency when filters or allParsedDraws change
    useEffect(() => {
        ...
        setFilteredDraws(currentFilteredDraws);
        if (currentFilteredDraws.length > 0) {
            setFrequencyCounts(calculateFrequency(currentFilteredDraws));
        } else {
            setFrequencyCounts([]);
        }
    }, [allParsedDraws, selectedMonth, selectedYear]);


    /**
     * Generates a single Mega-Sena game based on the chosen strategy.
     * @param {string} strategy The generation strategy ('frequencia' or 'ponderada').
     * @param {Array<[number, number]>} frequencies The sorted frequency counts.
     * @returns {Array<number>} A sorted array of 6 unique Mega-Sena numbers.
     */
    const generateSingleGame = (strategy, frequencies) => {
        let game = new Set();
        const availableNumbers = Array.from({ length: 60 }, (_, i) => i + 1); // Numbers 1-60

        if (strategy === 'frequencia') {
            // Get the top 6 most frequent numbers
            const topNumbers = frequencies.slice(0, 6).map(item => item[0]);
            game = new Set(topNumbers);

            // If not enough unique top numbers, fill with random unique numbers
            while (game.size < 6) {
                const remainingNumbers = availableNumbers.filter(num => !game.has(num));
                if (remainingNumbers.length === 0) break; // Should not happen for 1-60 range
                game.add(remainingNumbers[Math.floor(Math.random() * remainingNumbers.length)]);
            }
        } else if (strategy === 'ponderada') {
            // Create a weighted list of numbers based on their frequency
            ...
            // If no past draws or not enough unique numbers in weighted list, fall back to random
            if (weightedNumbers.length < 6) {
                ...
                while (game.size < 6) {
                    // Pick a random number from the weighted list
    ...
    /**
     * Handles the generation of multiple Mega-Sena games.
     */
    const handleGenerateGames = () => {
        if (filteredDraws.length === 0) {
            setErrorMessage('Nenhum sorteio encontrado para os filtros selecionados. Por favor, ajuste os filtros ou insira mais dados.');
            setGeneratedGames([]);
            return;
        }
        setErrorMessage('');
    ...
    // Memoize the most common numbers for display
    const mostCommonNumbers = useMemo(() => {
        return frequencyCounts.slice(0, 20); // Display top 20 most frequent
    }, [frequencyCounts]);

    // Get unique years from all parsed draws for the year filter dropdown
    const availableYears = useMemo(() => {
        const years = new Set(allParsedDraws.map(draw => draw.date.getFullYear().toString()));
        return ['all', ...Array.from(years).sort((a, b) => Number(b) - Number(a))]; // Sort descending
    }, [allParsedDraws]);


    return (
        <div className="min-h-screen bg-gradient-to-br from-green-400 to-blue-500 p-4 font-inter flex items-center justify-center">
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-4xl border-4 border-yellow-400">
                <h1 className="text-3xl md:text-4xl font-extrabold text-center text-gray-800 mb-4 drop-shadow-md">
                ...
                {/* Disclaimer Section */}
                <div className="bg-yellow-50 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-lg mb-6 shadow-inner">
                ...
                {/* How to Use Section */}
                <div className="mb-6 bg-blue-50 p-5 rounded-xl shadow-inner border border-blue-200">
                    <h2 className="text-xl md:text-2xl font-bold text-blue-800 mb-4">
                        Como Usar:
                    </h2>
                    <ol className="list-decimal list-inside text-gray-700 space-y-2">
                        <li>
                            <span className="font-semibold">Visualize os Sorteios Anteriores:</span> Os resultados dos concursos da Mega-Sena estão pré-carregados na caixa de texto abaixo.
                        </li>
                        <li>
                            <span className="font-semibold">Filtre por Mês e Ano:</span> Use os seletores de Mês e Ano para focar a análise em um período específico.
                        </li>
                        <li>
                            <span className="font-semibold">Verifique a Análise de Frequência:</span> O sistema calculará e exibirá os números mais frequentes com base nos dados filtrados.
                        </li>
                        <li>
                            <span className="font-semibold">Escolha as Opções de Geração:</span> Defina quantos jogos deseja gerar e qual estratégia usar (frequência direta ou ponderada).
                        </li>
                        <li>
                            <span className="font-semibold">Gere Suas Dezenas:</span> Clique no botão "Gerar Dezenas" para obter suas sugestões.
                        </li>
                    </ol>
                </div>

                {/* Input Section (now display-only) */}
                <div className="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner">
                    <label htmlFor="pastDraws" className="block text-gray-700 text-lg font-bold mb-3">
                        1. Resultados dos Sorteios Anteriores:
                    </label>
                    <textarea
                        id="pastDraws"
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 ease-in-out text-gray-800 h-60 resize-y"
                        value={pastDrawsInput}
                        readOnly // Make it read-only
                    ></textarea>
                    {errorMessage && (
                        <p className="text-red-600 text-sm mt-2 font-medium whitespace-pre-wrap">{errorMessage}</p>
                    )}
                </div>
                ...
                <div className="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mb-4">
                        <div className="flex-1">
                            <label htmlFor="filterMonth" className="block text-gray-700 text-md font-medium mb-2">
                                Mês:
                            </label>
                            <select
                                id="filterMonth"
                                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 ease-in-out text-gray-800 bg-white"
                                value={selectedMonth}
                        ...
                        <div className="flex-1">
                            <label htmlFor="filterYear" className="block text-gray-700 text-md font-medium mb-2">
                                Ano:
                            </label>
                            <select
                                id="filterYear"
                    ...
                    {filteredDraws.length === 0 && allParsedDraws.length > 0 && (
                ...
                {frequencyCounts.length > 0 && filteredDraws.length > 0 && (
                    <div className="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner">
                        <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4">
                            3. Análise de Frequência dos Números (Sorteios Filtrados):
                        </h2>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                            {mostCommonNumbers.map(([num, freq]) => (
                                <div key={num} className="bg-green-100 p-3 rounded-lg text-center shadow-sm border border-green-200">
                                    <span className="font-bold text-green-700 text-lg">{String(num).padStart(2, '0')}</span>
                                    <span className="text-gray-600 text-sm block">({freq} vezes)</span>
                                </div>
                            ))}
                        </div>
                        {frequencyCounts.length > 20 && ( // Changed from 10 to 20
                ...
                {/* Generated Games Section - Display filtered draws here */}
                    ...
                    <div className="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner">
                        ...
                        <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-lg p-3 bg-white">
                            {filteredDraws.map((draw, index) => (
                                <div key={index} className="mb-2 p-2 border-b border-gray-200 last:border-b-0">
                ...
                <div className="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                        <div className="flex-1">
                            <label htmlFor="numGames" className="block text-gray-700 text-md font-medium mb-2">
                                Quantos jogos gerar?
                            </label>
                            <input
                                type="number"
                                id="numGames"
                                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200 ease-in-out text-gray-800"
                                min="1"
                                max="10" // Limit to 10 games for practicality
                                value={numGamesToGenerate}
                                onChange={(e) => setNumGamesToGenerate(Math.max(1, Math.min(10, Number(e.target.value))))}
                            />
                        </div>
                        <div className="flex-1">
                            <label htmlFor="strategy" className="block text-gray-700 text-md font-medium mb-2">
                            ...
                            <select
                                ...
                                onChange={(e) => setGenerationStrategy(e.target.value)}
                            >
                    ...
                    <button
                        ...
                        className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-green-300"
                    >
                        Gerar Dezenas
                    </button>
                </div>
                ...
                {generatedGames.length > 0 && (
                    <div className="bg-green-50 p-5 rounded-xl shadow-lg border border-green-200">
                        <h2 className="text-xl md:text-2xl font-bold text-green-800 mb-4">
                            5. Seus Jogos Gerados:
                        </h2>
                        <div className="grid grid-cols-1 gap-4">
                            {generatedGames.map((game, index) => (
                                <div key={index} className="bg-white p-4 rounded-lg shadow-md border border-green-100 flex flex-wrap items-center justify-center gap-3">
                                    <span className="text-gray-700 font-semibold mr-2">Jogo {index + 1}:</span>
                                    {game.map((num, i) => (
                                        <span key={i} className="bg-green-500 text-white font-bold text-lg w-10 h-10 flex items-center justify-center rounded-full shadow-md">
