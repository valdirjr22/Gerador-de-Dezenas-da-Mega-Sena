...
<body class="min-h-screen bg-gradient-to-br from-green-400 to-blue-500 p-4 flex items-center justify-center">
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-4xl border-4 border-yellow-400">
        ...
        <div id="filteredDrawsSection" class="mb-6 bg-gray-50 p-5 rounded-xl shadow-inner hidden">
            <h2 class="text-xl md:text-2xl font-bold text-gray-800 mb-4">
                Sorteios Filtrados:
            </h2>
            <div id="filteredDrawsList" class="max-h-60 overflow-y-auto border border-gray-300 rounded-lg p-3 bg-white">
                </div>
        </div>
...
`
        ...
        const filteredDrawsSection = document.getElementById('filteredDrawsSection');
        const filteredDrawsListDiv = document.getElementById('filteredDrawsList');
        ...
        function parseDraws(rawData, numbersPerDraw) {
            ...
            for (const line of lines) {
                // Check for month/year headers (e.g., "Janeiro/2015")
                const monthYearMatch = line.match(/([a-zA-ZçÇãÃõÕéÉíÍóÓúÚ]+)\/(\d{4})/);
                if (monthYearMatch) {
                ...
                const drawMatch = line.match(/Concurso: (\d+) - (\d{2}\/\d{2}\/\d{4}) \(([^)]+)\)/);
                if (drawMatch) {
                    // The next line should contain the numbers
                    continue; // Skip this line, numbers are on the next
                }

                // If it's a line with numbers
                const numbers = line.split(' ').map(Number).filter(n => !isNaN(n) && n > 0);

                if (numbers.length === numbersPerDraw) {
                    // Extract month and year from the date string if available, otherwise use currentMonth/Year
                    const dateMatch = lines[lines.indexOf(line) - 1]?.match(/ - (\d{2}\/(\d{2})\/(\d{4}))/);
                    let drawMonth = currentMonth;
                    let drawYear = currentYear;

                    if (dateMatch) {
                        const monthNum = dateMatch[2];
                        const yearNum = dateMatch[3];
                        drawMonth = monthNumToName(monthNum);
                        drawYear = yearNum;
                    }
                    ...
                    // This means a line with numbers was found but it's not the expected count.
                    // This could be due to incomplete data or malformed input.
                    // For now, we'll just log and skip, but in a real app, you might want more robust error handling.
                    console.warn(`Skipping malformed line (unexpected number count for ${currentLottery}): ${line}`);
                }
            }
            return draws;
        }
        ...
        /**
         * Renders the filtered draws in the dedicated list.
         * @param {Array<Object>} draws - An array of filtered draw objects.
         */
        function renderFilteredDraws(draws) {
            filteredDrawsListDiv.innerHTML = ''; // Clear previous content
            if (draws.length === 0) {
                noDrawsMessage.classList.remove('hidden');
                filteredDrawsSection.classList.add('hidden');
                return;
            } else {
                noDrawsMessage.classList.add('hidden');
                filteredDrawsSection.classList.remove('hidden');
            }

            draws.forEach(draw => {
                const div = document.createElement('div');
                div.className = 'bg-white p-2 rounded-md shadow-sm border border-gray-100 text-sm';
                div.innerHTML = `<span class="font-semibold">${draw.month} ${draw.year}:</span> ${draw.numbers.map(n => String(n).padStart(2, '0')).join(' ')}`;
                filteredDrawsListDiv.appendChild(div);
            });
        }
        ...
        function applyFiltersAndRecalculate() {
            ...
            renderFilteredDraws(filteredDraws);
